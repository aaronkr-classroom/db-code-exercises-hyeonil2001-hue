...
5. 테이블 구조는 남겨두고 저장된 데이터만 삭제하는 명령 / TRUNCATE
6. 기본 테이블의 구조를 변경하는 명령 / ALATER TABLE
7. 기본 테이블로부터 만들어지는 가상 테이블인 뷰를 생성하는 영령 / CREATE VIEW
8. 기본 테이블 구조와 함께 저장된 데이터와 인텍스까지 한꺼번에 삭제하는 영령 / DROP TABLE
9. DDL 문으로 만들어지지 않고， 질의문 처리 괴정의 중간 결과로 만들어지는 테이블 / 임시 테이블
10. 기존 테이블에 새로운 데이터를 삽입하는 명렁 / INSERT
11. 시뭔스에 의해 지동으로 생성되는 가상의 열 / 의사열
12. CREATE VIEW 명령을 이용해서 기몬 테이블로부터 만들어지는 테이블 / VIEW
13. 단 하나의 테이블만 기초로 하여 생성된 뷰 / 단순 뷰
14. 테이블에서 기존 데이터를 갱신히는 명령 / UPDATE

**세로**

1. 시뭔스를 생성히는 명령 / CREATE SEQUENCE
2. 기존 뷰를 삭제하는 영렁 / DROP VIEW
3. 기존 테이블에 새로운 데이터를 삽입하는 명령 / INSERT
4. 테이블에서 기존 데이터를 삭제하는 명령 / DELETE
5. 기본 테이블을 생성하는 명령 / CREATE TABLE
6. 다음에 사용 가능한 시뭔스 값을 생성하는 의사열 / NEXTVAL
7. 고정길이 문자열을 기억히는 데이터 타입 / CHAR
8. 테이블에서 데이터를 검색하는 명렁 / SELECT
9. 독자적으로 존재하는 테이블 / 기본 테이블
10. 다중 테이블을 기초로 하여 생성된 뷰 / 복합 뷰
